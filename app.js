...
(function () {
  ...
  async function resolveIds(rows) {
    ...
    if (idSet.size === 0) {
      rows.forEach(r => { r.campaign = r.campaignRaw; r.adset = r.adsetRaw; r.ad = r.adRaw; });
      return;
    }
    ...
    for (let i = 0; i < idArr.length; i += CHUNK) {
      ...
      try {
        const res  = await fetch(`${FB_BASE}/?ids=${chunk}&fields=name&access_token=${token}`);
        const data = await res.json();
        if (data.error) { console.warn('[UTM resolve]', data.error.message); continue; }
        Object.entries(data).forEach(([id, obj]) => { if (obj && obj.name) nameMap[id] = obj.name; });
      } catch (e) { console.warn('[UTM resolve]', e.message); }
    }
  ...
  function renderUTMResults(rows) {
    ...
    document.getElementById('utm-table-body').innerHTML = rows.map(r => `
      <tr>
        ...
        <td>${r.campaign || '<span style="color:var(--text-faint)">—</span>'}</td>
        <td>${r.adset    || '<span style="color:var(--text-faint)">—</span>'}</td>
        <td>${r.ad       || '<span style="color:var(--text-faint)">—</span>'}</td>
  ...
  document.getElementById('utm-analyze-btn').addEventListener('click', async () => {
    ...
    btn.textContent = 'Resolving IDs…'; btn.disabled = true;
    try { await resolveIds(rows); } finally { btn.textContent = 'Parse UTMs'; btn.disabled = false; }
  ...
  document.getElementById('utm-export-btn').addEventListener('click', () => {
    ...
    const a = document.createElement('a');
    a.href = 'data:text/csv,' + encodeURIComponent(csv);
    a.download = 'utm-lookup.csv'; a.click();
  });
